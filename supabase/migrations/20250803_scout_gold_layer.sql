-- ====================================================================
-- SCOUT DASHBOARD GOLD LAYER - BUSINESS INTELLIGENCE VIEWS
-- ====================================================================
-- Executive-ready, pre-aggregated analytics for Scout Dashboard
...
-- ====================================================================
-- Note: These indexes should already exist on the silver tables
-- but we ensure they're present for optimal Gold layer performance

CREATE INDEX IF NOT EXISTS idx_silver_transactions_date 
ON scout.silver_transactions_cleaned(transaction_date);

CREATE INDEX IF NOT EXISTS idx_silver_transactions_store 
ON scout.silver_transactions_cleaned(store_id);

CREATE INDEX IF NOT EXISTS idx_silver_transactions_brand 
ON scout.silver_transactions_cleaned(brand_name);

CREATE INDEX IF NOT EXISTS idx_silver_transactions_location 
ON scout.silver_transactions_cleaned((location->>'city'));

-- ====================================================================
-- REFRESH FUNCTION FOR MATERIALIZED VIEWS (IF NEEDED)
-- ====================================================================
CREATE OR REPLACE FUNCTION gold.refresh_scout_dashboard_stats()
RETURNS void AS $$
BEGIN
  -- This function can be used to refresh materialized views if we convert
  -- any of the above views to materialized views for better performance
  RAISE NOTICE 'Scout Dashboard Gold Layer views refreshed at %', NOW();
END;
$$ LANGUAGE plpgsql;
...
GRANT SELECT ON ALL VIEWS IN SCHEMA gold TO anon, authenticated;

-- Add comments for documentation
COMMENT ON VIEW gold.scout_dashboard_executive IS 'Executive KPIs and high-level metrics for Scout Dashboard';
COMMENT ON VIEW gold.scout_dashboard_regions IS 'Regional performance metrics with growth rates and market tiers';
COMMENT ON VIEW gold.scout_dashboard_transactions IS 'Transaction analytics including daily trends and hourly patterns';
COMMENT ON VIEW gold.scout_dashboard_brands IS 'Brand and campaign performance metrics';
COMMENT ON VIEW gold.scout_dashboard_locations IS 'Store and venue level analytics with opportunity scoring';
COMMENT ON VIEW gold.scout_dashboard_customers IS 'Customer segmentation and behavioral analytics';
COMMENT ON VIEW gold.scout_dashboard_predictions IS 'Predictive analytics and growth projections';