-- Geographic Analytics Dashboard - Fix MultiPolygon Type Mismatch
-- Project: cxzllzyxwpyptfretryc
-- Run this in Supabase SQL Editor: https://app.supabase.com/project/cxzllzyxwpyptfretryc/sql/new

-- First, check the actual geometry type in geo.admin2
SELECT column_name, udt_name, 
       geometry_columns.type as geom_type,
       geometry_columns.srid
FROM information_schema.columns
LEFT JOIN geometry_columns ON (
    columns.table_schema = geometry_columns.f_table_schema 
    AND columns.table_name = geometry_columns.f_table_name 
    AND columns.column_name = geometry_columns.f_geometry_column
)
WHERE columns.table_schema = 'geo' 
AND columns.table_name = 'admin2'
AND columns.column_name IN ('geometry', 'geom');
...
-- Insert Philippine regions with ST_Multi() to convert POLYGON to MULTIPOLYGON
